...
async function handleStartAttempt(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await logUserHistoryAction({
      userId: payload.userId,
      actionType: 'PAPER_ATTEMPT_STARTED',
      title: 'Started past year paper',
      description: `Started ${paper.title}`,
      metadata: { paperId: paper.id, paperTitle: paper.title, attemptId: attempt.id },
    });
...
async function handleSubmitAttempt(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    for (const question of attempt.paper.questions) {
      ...
      updateTopicStats(parseTopics(question.topics), { isCorrect, isAttempted, scoreDelta });
    }
