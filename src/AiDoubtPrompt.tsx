...
import { renderRichTextWithLatex } from './utils/latex';
import { API_BASE } from './lib/apiBase';

export function AiDoubtPrompt({
  questionId,
  aiSolution,
  apiPath = '/z7i?action=ai-doubt',
  questionPayload,
}: {
  questionId: string;
  aiSolution: string;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}${apiPath}`, {
        ...
        body: JSON.stringify({ questionId, aiSolution, doubt, model, question: questionPayload }),
      });
        ...
        <div className="ai-doubt-response-panel">
          ...
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: DOMPurify.sanitize(renderRichTextWithLatex(response))
            }}
          />
        </div>
      )}
