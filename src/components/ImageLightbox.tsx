    ...
    <div className="lightbox-overlay" onClick={onClose}>
      ...
      <div className="lightbox-content" onClick={(event) => event.stopPropagation()}>
        <div className="lightbox-media">
          <img ref={imageRef} src={src} alt="Enlarged view" className={`lightbox-image ${shouldInvert ? 'invert' : ''}`.trim()} onLoad={resizeCanvas} />
          <canvas
            ref={canvasRef}
...
function isGifSource(src: string) {
  ...
  return normalized.startsWith('data:image/gif;') || normalized.includes('.gif');
}

function shouldInvertExpandedImage(source: HTMLImageElement) {
  const container = source.closest('.invert-images');
...
function useImageLightbox(context?: LightboxContext) {
  const [lightboxState, setLightboxState] = useState<{ src: string; context?: LightboxContext; shouldInvert: boolean } | null>(null);
  ...
  const handleImageClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (image instanceof HTMLImageElement) {
        ...
        const shouldInvert = shouldInvertExpandedImage(image);
        setLightboxState({ src, context, shouldInvert });
      }
    },
    [context]
  );
