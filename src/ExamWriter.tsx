...
function getReattemptProgressKey(attemptId: string) {
  return `z7i-reattempt-progress-${attemptId}`;
}
...
function QuestionStatusBadge({ 
  ...
  return (
    <button 
      ...
      onClick={onClick}
    >
      <span className="exam-nav-badge-number">Q{number}</span>
...
export function ExamWriter({ test, onBack, onViewAnalysis, onSubmitted }: ExamWriterProps) {
  ...
  const userAnswersRef = useRef<Map<string, UserAnswer>>(new Map());
  ...
  const flushCurrentQuestionTime = useCallback(() => {
    ...
    setUserAnswers(prev => {
      ...
      if (existing) {
        next.set(questionId, { ...existing, timeSpent: existing.timeSpent + timeSpent });
      }
      return next;
    });
  ...
  const persistProgress = useCallback((override?: { forceElapsedTime?: number }) => {
    if (questions.length === 0 || examFinished) return;
    const payload = {
      currentIndex: currentIndexRef.current,
      elapsedTime: override?.forceElapsedTime ?? elapsedTimeRef.current,
      userAnswers: Array.from(userAnswersRef.current.entries()),
      updatedAt: new Date().toISOString(),
    };
    localStorage.setItem(getReattemptProgressKey(test.id), JSON.stringify(payload));
  }, [questions.length, examFinished, test.id]);
    ...
    const loadQuestions = async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          const savedProgressRaw = localStorage.getItem(getReattemptProgressKey(test.id));
          if (savedProgressRaw) {
            try {
              const savedProgress = JSON.parse(savedProgressRaw) as {
                currentIndex?: number;
                elapsedTime?: number;
                userAnswers?: Array<[string, UserAnswer]>;
              };
              if (Array.isArray(savedProgress.userAnswers)) {
                for (const [questionId, answerState] of savedProgress.userAnswers) {
                  if (initial.has(questionId)) {
                    initial.set(questionId, { ...initial.get(questionId)!, ...answerState, questionId });
                  }
                }
              }
              if (typeof savedProgress.currentIndex === 'number') {
                setCurrentIndex(Math.max(0, Math.min(savedProgress.currentIndex, Math.max(0, sortedQuestions.length - 1))));
              }
              if (typeof savedProgress.elapsedTime === 'number' && savedProgress.elapsedTime >= 0) {
                setElapsedTime(savedProgress.elapsedTime);
              }
            } catch (progressError) {
              console.error('Failed to parse reattempt progress:', progressError);
            }
          }
  ...
  useEffect(() => {
    ...
    persistProgress();
  }, [loading, examFinished, persistProgress, userAnswers, currentIndex, elapsedTime]);

  useEffect(() => {
    const handleBeforeUnload = () => {
      flushCurrentQuestionTime();
      persistProgress({ forceElapsedTime: elapsedTimeRef.current });
    };
    window.addEventListener('beforeunload', handleBeforeUnload);
    return () => window.removeEventListener('beforeunload', handleBeforeUnload);
  }, [flushCurrentQuestionTime, persistProgress]);
  ...
  }, [currentIndex, questions, flushCurrentQuestionTime]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    localStorage.removeItem(getReattemptProgressKey(test.id));
    await onSubmitted?.();
  ...
  }, [questions, userAnswers, elapsedTime, test.totalScore, test.id, flushCurrentQuestionTime, onSubmitted]);
  ...
  return (
    <div className="exam-writer">
      ...
      {lightboxState && (
        <ImageLightbox src={lightboxState.src} shouldInvert={lightboxState.shouldInvert} onClose={closeLightbox} />
      )}
    </div>
  );
}
